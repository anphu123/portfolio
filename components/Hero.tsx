...
const DoodleHeart = () => (
  <svg style={{ display: 'inline-block', verticalAlign: 'middle', animation: 'wiggle 3s ease-in-out infinite' }} width="22" height="20" viewBox="0 0 24 22" fill="none">
    <path d="M12,19 C12,19 2,12 2,6 C2,3 4,1 7,1 C9,1 11,2.5 12,4 C13,2.5 15,1 17,1 C20,1 22,3 22,6 C22,12 12,19 12,19 Z" stroke="#F45B69" strokeWidth="2" fill="none" strokeLinejoin="round"/>
  </svg>
);
...
const DoodleStar = ({ style }: { style?: React.CSSProperties }) => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" style={{ display: 'inline-block', ...style }}>
    <path d="M10,2 L11.8,7.6 L18,7.6 L13,11.2 L15,17 L10,13.5 L5,17 L7,11.2 L2,7.6 L8.2,7.6 Z" stroke="#FFC108" strokeWidth="1.8" fill="none" strokeLinejoin="round"/>
  </svg>
);
    ...
    <section className="grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-6">
      ...
      >
        ...
        <div style={{
          ...
          width: 70, height: 20,
          background: 'repeating-linear-gradient(45deg, rgba(1,117,194,0.35), rgba(1,117,194,0.35) 4px, rgba(84,197,248,0.35) 4px, rgba(84,197,248,0.35) 8px)',
          borderRadius: 3, transform: 'rotate(-1.5deg)', zIndex: 2,
        }} />

        {/* BG doodle */}
        <svg className="absolute top-4 right-4 opacity-5 pointer-events-none" width="160" height="160" viewBox="0 0 24 24" fill="none">
          <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" stroke="#1a2744" strokeWidth="0.8"/>
        </svg>
          ...
          <div className="flex flex-col items-start flex-1">
            ...
            >
              <span className="relative flex h-2.5 w-2.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full" style={{ background: '#00C853', opacity: 0.7 }} />
                <span className="relative inline-flex rounded-full h-2.5 w-2.5" style={{ background: '#00C853' }} />
              </span>
              <span style={{ fontSize: '0.78rem', color: '#0175C2', fontWeight: 600 }}>
                {t.ui.hero.available}
              </span>
            </div>

            {/* Name — Caveat bold (Vietnamese safe) */}
            <h1
            ...
            <div className="relative shrink-0 order-first md:order-last mb-4 md:mb-0 float-3d">
              ...
              >
                <ellipse cx="55" cy="55" rx="50" ry="52"
                  stroke="#54C5F8" strokeWidth="2" fill="none" strokeDasharray="8 5" opacity="0.7"
        ...
        {/* Summary */}
        <p className="leading-relaxed mb-8 max-w-xl" style={{ fontFamily: "'Patrick Hand', cursive", fontSize: '1rem', color: '#37474F', lineHeight: 1.7 }}>
          {summary} <DoodleHeart />
        </p>
        ...
        <div className="flex flex-wrap gap-3 print:hidden">
          {CONTACT_DATA.resumeUrl ? (
            <a
              ...
              style={{ background: 'linear-gradient(135deg, #0175C2, #54C5F8)', color: '#fff', fontSize: '0.95rem' }}
            >
          ...
          <a
            ...
            className="btn-doodle inline-flex items-center gap-2 px-5 py-2.5"
            style={{ background: '#FFFFFF', color: '#1a2744', fontSize: '0.95rem' }}
          >
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          ...
          <h3 className="mb-6" style={{ fontFamily: "'Caveat', cursive", fontSize: '0.85rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#546E7A' }}>
            {t.ui.hero.contactInfo}
          </h3>

          <ul className="space-y-5">
            {[
              { icon: <PhoneIcon className="w-5 h-5" />, label: t.ui.hero.phone, value: CONTACT_DATA.phone, href: `tel:${CONTACT_DATA.phone.replace(/\s/g, '')}`, color: '#0175C2' },
              { icon: <MailIcon className="w-5 h-5" />, label: t.ui.hero.email, value: CONTACT_DATA.email, href: `mailto:${CONTACT_DATA.email}`, color: '#00B4AB' },
              { icon: <MapPinIcon className="w-5 h-5" />, label: t.ui.hero.address, value: CONTACT_DATA.address, href: null, color: '#F45B69' },
            ].map(({ icon, label, value, href, color }) => (
              <li key={label} className="flex items-start gap-4 group">
                <div style={{ padding: 8, border: `2px solid ${color}`, borderRadius: '8px 12px 10px 14px', color, background: `${color}15`, boxShadow: `2px 2px 0 ${color}55`, flexShrink: 0 }}>
                  {icon}
                </div>
                <div>
                  <span className="block mb-0.5" style={{ fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#546E7A', fontWeight: 700, fontFamily: "'Patrick Hand', cursive" }}>
                    {label}
                  </span>
                  {href ? (
                    <a href={href} style={{ fontSize: '0.9rem', color: '#1a2744', fontFamily: "'Caveat', cursive", fontWeight: 600, textDecoration: 'none' }}>{value}</a>
                  ) : (
                    <span style={{ fontSize: '0.9rem', color: '#1a2744', fontFamily: "'Caveat', cursive", fontWeight: 600 }}>{value}</span>
                  )}
                </div>
              </li>
            ))}
          ...
          <svg className="mt-6 w-full" height="14" viewBox="0 0 200 14" fill="none" style={{ opacity: 0.35 }}>
            <path d="M0,7 Q25,2 50,7 Q75,12 100,7 Q125,2 150,7 Q175,12 200,7" stroke="#546E7A" strokeWidth="2" strokeLinecap="round" fill="none"/>
          </svg>
        </div>
      </div>
    </section>
